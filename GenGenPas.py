...
class GenGenPas:
    ...
    def get_input(self):
        while self.is_alive:
            ...
            print('[+] To generate password dictionary just tap: run')
    ...
    def generate(self):
        ...
        with open(save_file, 'wt', encoding='utf-8') as output_file:
            print(f'Generated a list of {len(self.password_list)} passwords')
            for pwd in self.password_list:
                output_file.write(f'{pwd}\n')


if __name__ == '__main__':
    save_file:str = input('Save file name (you can also tap path to this file example -> ~/home/usr/www/pass.txt): ')
    GenGenPas(save_file).get_input()
